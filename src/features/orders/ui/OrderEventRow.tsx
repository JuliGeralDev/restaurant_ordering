...
const EventDetail = ({
  ...
  if (["CART_ITEM_ADDED", "CART_ITEM_UPDATED", "CART_ITEM_REMOVED"].includes(type)) {
    return (
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-[10px]">
          {payload.subtotal != null && (
            <span className="text-zinc-500">
              Subtotal{" "}
              <span className="font-semibold text-zinc-300">{fmt(Number(payload.subtotal))}</span>
            </span>
          )}
          {payload.tax != null && (
            <span className="text-zinc-500">
              Tax{" "}
              <span className="font-semibold text-zinc-300">{fmt(Number(payload.tax))}</span>
            </span>
          )}
          {payload.serviceFee != null && (
            <span className="text-zinc-500">
              Service fee{" "}
              <span className="font-semibold text-zinc-300">{fmt(Number(payload.serviceFee))}</span>
            </span>
          )}
        </div>
        {payload.total != null && (
          <div className="mt-1.5 flex items-center justify-between border-t border-zinc-800 pt-1.5">
            ...
            <span className="text-[14px] font-bold text-blue-400">{fmt(Number(payload.total))}</span>
          </div>
        )}
      </div>
    );
  ...
  // generic fallback: show all unknown payload fields
  return (
